...
class GE2E(torch.nn.Module):
    ...
    def forward(self, mels: torch.Tensor, samples: int= 1):
        '''
        mels: [Batch * Sample, Mel_dim, Time]
        '''
        x = self.prenet(mels)   # [Batch * Sample, Emb_dim, Time]
        x = self.relu(x)
        x = self.positional_embedding(x)    # [Batch * Sample, Emb_dim, Time]
        x, _ = self.transformer(x.permute(2, 0, 1)) # [Time, Batch * Sample, Emb_dim]
        x = x.permute(1, 2, 0)[:, :, -1:] # [Batch * Sample, Emb_dim, 1], Use only last time
        x = self.projection(x).squeeze(2)   # [Batch * Sample, Emb_dim]
